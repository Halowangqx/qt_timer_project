...
{
    pressed = true;
    pressedPos = horizontal ? e->pos().x() : e->pos().y();
}

void Tumbler::mouseMoveEvent(QMouseEvent *e)
{
    int maxIndex = listValue.count() - 1;
    if (maxIndex <= 0) {
        return;
    }

    int pos = horizontal ? e->pos().x() : e->pos().y();
    int len = horizontal ? this->width() : this->height();
    int index = listValue.indexOf(currentValue);

    if (pressed) {
        //数值到边界时,阻止继续往对应方向移动
        if ((index == 0 && pos >= pressedPos) || (index == maxIndex && pos <= pressedPos)) {
            return;
        }
        ...
        //若移动速度过快时进行限制
        if (offset > len / percent) {
            offset = len / percent;
        } else if (offset < -len / percent) {
            offset = -len / percent;
        }
...
void Tumbler::mouseReleaseEvent(QMouseEvent *e)
{
    if (pressed) {
        pressed = false;
        releasePos = horizontal ? e->pos().x() : e->pos().y();
        int len = horizontal ? this->width() : this->height();
        ...
        //判断单击位置,当按下处的坐标和松开处的坐标一致才认为是单击,上部分移动元素-1,下部分移动元素+1
        if (clickEnable && pressedPos == releasePos) {
            if (pressedPos <= len / 3) {
                if (currentIndex > 0) {
                    setCurrentIndex(currentIndex - 1);
                    emit clicked();
                }
            } else if (pressedPos >= len * 2 / 3) {
...
bool Tumbler::getClickEnable() const
{
...
void Tumbler::setClickEnable(bool clickEnable)
{
